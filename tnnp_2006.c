...
double G_Kr = 1.0*0.153; // Maximal I_Kr (rapidly activating delayed rectifier potassium current) conductance -> nS/pF
...
double G_Ks = 0.2*0.098;        // Maximal I_Ks (slowly activating delayed rectifier potassium current) conductance -> nS/pF (M cells) (Sachetto)
double p_KNa = 0.03;        // Relative I_Ks permeability to Na+ over K+ -> dimensionless
double G_CaL = 2.35*2*3.98e-5;     // Maximal I_CaL (L-type calcium current) conductance -> cm/ms/uF
...
{
    return 0.5*(alpha_f(V) + beta_f(V) + gamma_f(V));
}
...
double simulation_time = 1500; // End time -> ms
...
int main(int argc, char *argv[])
{
    ...
    double V = V_init;
    double X_r1 = X_r1_init;
    double X_r2 = X_r2_init;
    double X_s = X_s_init;
    double m = m_init;
    double h = h_init;
    double j = j_init;
